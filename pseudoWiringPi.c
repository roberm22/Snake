...
#define BACKUP_NEWLINE "\033[A\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C"
		...
		if(columnaDisplayActiva == 7 && pin == GPIO_LED_DISPLAY_ROW_7) { // Esta condicion equivale a comprobar cuando se termina de hacer un barrido COMPLETO del display
			...
			if(JUGANDO) {
				...
				if (!primeraVezDisplay) { // Codigo que permite repintar la pantalla SIN SCROLL VERTICAL HACIA ABAJO
					...
					printf("\033[A");
					printf("\033[A\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C\033[C");

				}

				//completamos aux matriz v5
				for(j=0;j<7;j++) {
					for(i=0;i<8;i++){
						if(pseudoMatriz[i][j] == 1)
							auxMatriz[i][j] = 'o';
						else{
							auxMatriz[i][j] = ' ';
						}
				...
				// Actualizo pantalla
				printf("\n[PANTALLA] \t Score: %d  \t Timer: \033[5m %d min %d seg \033[0m \n\n", valores, tiempoMin, tiempoSec);
				printf ("\033[0;32m"); //v5
				printf("╔════════╗ ");
				printf("\033[0m");
				printf("\t \033[4mName\tScore\t\033[0m   \033[4mTime\033[0m \n");
				...
				for(j=0;j<7;j++) {
					for(i=0;i<8;i++){
						...
						if( posicionMan[0] == i && posicionMan[1] == j ){
							printf ("\033[1;31m"); //v5
							printf("%c", auxMatriz[i][j]);
							printf("\033[0m");
						}else{
						printf ("\033[1;34m"); //v5
						printf("%c", auxMatriz[i][j]);
						...
						if (i == 7) {
							...
							printf("\033[0m");
							if (j == 0) {
								printf("\t   %c\t  %d\t %d min %d sec ", nombres[0][0],
										puntuaciones[0][0], puntuaciones[0][1], puntuaciones[0][2]);
							}
							if (j == 1) {
								printf("\t   %c\t  %d\t %d min %d sec ", nombres[1][0],
										puntuaciones[1][0], puntuaciones[1][1], puntuaciones[1][2]);
							}
							if (j == 2) {
								printf("\t   %c\t  %d\t %d min %d sec ", nombres[2][0],
										puntuaciones[2][0], puntuaciones[2][1], puntuaciones[2][2]);
							}
						}
					}
					if(j<6)
