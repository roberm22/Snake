...
int ActualizaSnakePi(tipo_snakePi *p_snakePi) {
	...
	for(int j=0;j<7;j++) {
		for(int i=0;i<8;i++) {
			if(led_display->pantalla.matriz[i][j])
				valores++;
		}
	}
	valores = valores -2;*/
	...
	if (comprueba_manzana) {
		...
		for(seg_i = serpiente->p_cola; seg_i->p_next; seg_i=seg_i->p_next) {
			if (serpiente->cabeza.x == seg_i->x && serpiente->cabeza.y == seg_i->y){
				//led_display.pantalla = pantalla_final;//v4
				return 1;
			}
		}
...
void PintaManzana(tipo_snakePi *p_snakePi) {
	p_snakePi->p_pantalla->matriz[p_snakePi->manzana.x][p_snakePi->manzana.y] = 1; //v5
	posicionMan[0] = p_snakePi->manzana.x; //v5
	posicionMan[1] = p_snakePi->manzana.y;  //v5
...
void ReseteaPantallaSnakePi(tipo_pantalla *p_pantalla) {//revisar
	// A completar por el alumno
	// ...
	int i,j;
...
void InicializaJuego(fsm_t* this) {
	...
	printf("\nPon una letra como usuario: ");
	scanf("%c", &(nombreactual));
	fflush(stdout);
	...
	p_snakePi = (tipo_snakePi*)(this->user_data);
	 ...
	 pseudoWiringPiEnableDisplay(1); //linea nueva version 4
	 led_display.pantalla = pantalla_inicial;//v4
}
...
void FinalJuego (fsm_t* this) {
	 ...
	 //led_display.pantalla = pantalla_final;
	  //piLock(MATRIX_KEY);
	  //ActualizaPantallaSnakePi(p_snakePi);
	  //piUnlock(MATRIX_KEY);
	 ...
	 jugando = 0;
	 pseudoWiringPiEnableDisplay(0); //linea nueva v4
	 printf("¡¡FIN DE JUEGO!!");
	 fflush(stdout);
}
...
void Clasificacion(){
			...
			if (valores > puntuaciones[aux][0]) {

				if(nombres[0][0] == nombreactual){
					puntuaciones[0][0] = valores;
					puntuaciones[0][1] = tiempoMin;
					puntuaciones[0][2] = tiempoSec;
					return;
				}

				nombres[aux+1][0] = nombres[aux][0];
				puntuaciones[aux+1][0] = puntuaciones[aux][0];
				puntuaciones[aux+1][1] = puntuaciones[aux][1];
				puntuaciones[aux+1][2] = puntuaciones[aux][2];

				nombres[aux][0] = nombreactual;
				puntuaciones[aux][0] = valores;
				puntuaciones[aux][1] = tiempoMin;
				puntuaciones[aux][2] = tiempoSec;
